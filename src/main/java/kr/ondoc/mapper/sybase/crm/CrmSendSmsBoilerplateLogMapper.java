...
public interface CrmSendSmsBoilerplateLogMapper {
	@Select("<script>"
			+ "SELECT * from crm_send_sms_boilerplate_log "
			+ "WHERE 0=0 "
			+ "AND log_idx = #{log_idx}"
			+ "</script>")
	...
	@Insert("<script>"
			+ "INSERT INTO crm_send_sms_boilerplate_log ("
			+ "reserved_id, status, send_datetime, sms_message, "
			+ "write_date,"
			+ "write_id) "
			+ "VALUES ( "
			+ "#{reserved_id}, #{status}, #{send_datetime}, #{sms_message}, "
	...
	@Delete("<script>"
			+ "UPDATE crm_send_sms_boilerplate_log SET "
